...
const Blog: React.FC = () => {
	...
	const { data: featuredPosts = [], isLoading: featuredLoading } =
		useFeaturedPosts(3, currentLanguage);
	...
	const getTitle = (post: Post): string => {
		const postWithLocalized = post as Post & {
			localized?: { title?: string; content?: string };
		};
									...
									{postsData && postsData.totalPages > 1 && (
										<motion.div
											...
											<button
												onClick={() =>
													setCurrentPage((prev) =>
														Math.min(postsData.totalPages, prev + 1)
													)
												}
										...
										validPosts.length === 0 && (
											<div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4 max-w-md mx-auto'>
												<p className='text-sm text-yellow-800'>
													<strong>{t("note")}:</strong> {postsData.posts.length}{" "}
													{t("postsFilteredOut")}
												</p>
											</div>
										)}
