...
import { Search, MapPin, Phone, User, Award } from "lucide-react";
...
const Doctors: React.FC = () => {
	...
	// Filter doctors by memberType === "partner"
	const doctors = allDoctors.filter(
		(doctor: any) => doctor.memberType === "partner"
	);
	...
	const getLocalizedText = (value: unknown): string => {
		...
		if (typeof value === "object") {
			...
			return (
				valueObj[currentLanguage as "en" | "ar"] ||
				valueObj.en ||
				valueObj.ar ||
				""
			);
				...
				<div className='container mx-auto px-4 sm:px-6 lg:px-8'>
					{isLoading ? (
						...
						<motion.div
							...
							<p className='text-gray-600'>{t("tryAdjustingYourFilters")}</p>
						</motion.div>
					) : (
								...
								<motion.div
									...
									<Link
										...
										<div className='bg-white rounded-2xl shadow-lg hover:shadow-xl hover:shadow-primary-500/20 transition-all duration-500 overflow-hidden border border-gray-100 h-full'>
											...
											<div className='p-6'>
												<motion.h3
													className={`text-xl font-bold text-gray-900 mb-2 group-hover:text-primary-700 transition-colors duration-300 ${
												...
												<motion.p
													className={`text-primary-600 font-semibold mb-3 group-hover:text-primary-700 transition-colors duration-300 ${
														isRTL ? "text-right" : "text-left"
													}`}>
												...
												<div
													className={`w-full bg-gradient-to-r from-primary-500 to-primary-600 text-white py-3 px-6 rounded-xl font-semibold transition-all duration-300 transform flex items-center justify-center ${
														isRTL ? "space-x-reverse space-x-2" : "space-x-2"
													}`}>
