...
const DoctorDetail: React.FC = () => {
	...
	const getLocalizedArray = (
		value: Array<{ en: string; ar: string }> | undefined
	): string[] => {
		if (!value) return [];
		if (Array.isArray(value)) {
			return value.map((item: { en: string; ar: string }) =>
	...
	const qualifications = doctorData?.qualifications
		? getLocalizedArray(doctorData.qualifications)
		: [];
	const experience = doctorData?.experience
		? getLocalizedArray(doctorData.experience)
		: [];
					...
					<div className='grid grid-cols-1 lg:grid-cols-3 gap-12'>
						...
						<div className='space-y-8'>
							...
							<motion.div
								...
								{doctorData?.socialMedia &&
									doctorData.socialMedia.length > 0 && (
										<div className='mt-6'>
											<a
												href={doctorData.socialMedia[0]}
												target='_blank'
												rel='noopener noreferrer'
												className={`inline-flex items-center bg-primary-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-primary-700 transition-colors duration-200 ${
													isRTL ? "flex-row-reverse gap-3" : "gap-2"
												}`}
												aria-label={
													currentLanguage === "ar" ? "تواصل معي" : "Contact Me"
												}>
