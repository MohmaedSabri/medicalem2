...
const Partners: React.FC = () => {
	...
	const { data: doctorsData } = useDoctors({ lang: currentLanguage });

	// Filter doctors by memberType === "partner"
	const partnerDoctors = (doctorsData ?? []).filter(
		(doctor) => doctor.memberType === "partner"
	);

	// Use partner doctors if available, otherwise fallback to contact info partners, then sample data
	const partnerLogos =
			...
			? partnerDoctors.map((doctor) => ({
					...
					title:
						typeof doctor.title === "object"
							? doctor.title.en || doctor.title.ar
							: doctor.title,
					href: `/partners/${doctor._id}`,
			  }))
			: (contactInfo as ContactInfo)?.partners?.length > 0
		...
		<section
			...
			<div className='container mx-auto px-4 sm:px-6 lg:px-8 relative z-10'>
				...
				<motion.div
					...
					<h2
						className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-primary-500 mb-3 sm:mb-4 md:mb-6 text-center`}>
						{t("ourPartners")}
					</h2>
					<p
						className={`text-sm sm:text-base md:text-lg text-gray-600 max-w-2xl lg:max-w-3xl mx-auto leading-relaxed text-center`}>
