...
const EditDoctorForm: React.FC<EditDoctorFormProps> = ({ doctor, onClose }) => {
  ...
  const defaultValues: Partial<DoctorCreateSchema> = {
    ...
    nameAr: (typeof doctor.name === 'object' ? (doctor.name as { ar?: string }).ar : toText(doctor.name)) || '',
    titleEn: toText((doctor.title as { en?: string }).en) || toText(doctor.title),
    titleAr: (typeof doctor.title === 'object' ? (doctor.title as { ar?: string }).ar : toText(doctor.title)) || '',
    descriptionEn: toText((doctor.description as { en?: string }).en) || toText(doctor.description),
    descriptionAr: (typeof doctor.description === 'object' ? (doctor.description as { ar?: string }).ar : toText(doctor.description)) || '',
    image: doctor.image || '',
    skills: (doctor.skills || []).map((s: unknown) => toText(s)),
    qualifications: (doctor.qualifications || []).map((q: unknown) => toText(q)),
    experience: (doctor.experience || []).map((e: unknown) => toText(e)),
    locationEn: toText((doctor.location as { en?: string }).en) || toText(doctor.location),
    locationAr: (typeof doctor.location === 'object' ? (doctor.location as { ar?: string }).ar : toText(doctor.location)) || '',
    contact: doctor.contact || '',
    socialMedia: doctor.socialMedia || [],
    specializationEn: toText((doctor.specialization as { en?: string }).en) || toText(doctor.specialization),
    specializationAr: (typeof doctor.specialization === 'object' ? (doctor.specialization as { ar?: string }).ar : toText(doctor.specialization)) || '',
  };
  ...
  const handleSubmit = async (data: DoctorCreateSchema) => {
    const payload = {
      ...
      qualifications: (data.qualifications || []).map((q) => ({ en: q || '', ar: q || '' })),
        ...
        >
          ...
          <div className="p-6 space-y-8">
            ...
            <GenericForm<DoctorCreateSchema>
              ...
              submitButtonText={updateDoctorMutation.isPending ? t('saving') : t('updateDoctor')}
              className='w-full'
            />
