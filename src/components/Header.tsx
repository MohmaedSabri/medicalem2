...
const Header: React.FC<HeaderProps> = ({ isLoginPage = false }) => {
  ...
  const navigationItems = [
    { path: '/', label: currentLanguage === 'ar' ? 'دورر' : 'Dorar', icon: Home },
    { path: '/blog', label: currentLanguage === 'ar' ? 'المدونة' : 'Blog', icon: BookOpen },
    { path: '/about', label: currentLanguage === 'ar' ? 'من نحن' : 'About', icon: Info },
    { path: '/favorites', label: currentLanguage === 'ar' ? 'المفضلة' : 'Favorites', icon: Heart },
    { path: '/contact', label: currentLanguage === 'ar' ? 'اتصل بنا' : 'Contact', icon: Phone },
  ];
  ...
  const contactFloatingVariants = {
	...
	return (
		...
		>
          ...
          <Link 
            to="/" 
            className={`flex items-center z-50 group flex-shrink-0 ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}
                ...
                const isActive = location.pathname === item.path;
                  ...
                  <React.Fragment key={item.path}>
                    ...
                    >
                      ...
                      {isActive && <Zap className="h-4 w-4 text-yellow-400 ml-1" />}
                    </Link>
                    ...
                    {index === 0 && (
                      ...
                      >
                        ...
                        <AnimatePresence>
									{showProductsMenu && (
										<div 
                              className={`products-dropdown absolute ${isRTL ? 'right-0' : 'left-0'} mt-1 w-96 rounded-2xl bg-white/95 backdrop-blur-md border border-white/40 shadow-2xl p-4 z-50`}
                              style={{ 
                            ...
                            >
                              ...
                              <div className='grid grid-cols-2 gap-2'>
																				{categories.length > 0 ? (
                                  categories.map((category: { _id: string; name: string | { en: string; ar: string } }) => {
                                    ...
                                    const categoryName = typeof category.name === 'object' 
                                      ...
                                      >
                                          ...
                                          <div className='font-medium text-sm'>
                                            <span className={isArabicText(categoryName) ? 'text-right' : 'text-left'}>
																											{categoryName}
																										</span>
                                          </div>
                                          <div className='text-xs text-gray-500 mt-1 flex items-center justify-between'>
                                            <span>{currentLanguage === 'ar' ? 'عرض المنتجات' : 'View Products'}</span>
																										{categorySubcategories.length > 0 && (
                                              <span className="text-teal-600 font-medium">
                                                {categorySubcategories.length} {currentLanguage === 'ar' ? 'فرعية' : 'sub'}
																											</span>
																										)}
																									</div>
																								</button>
                                        ...
                                        {/* Subcategories dropdown on hover */}
                                            ...
                                            >
                                                ...
                                                {currentLanguage === 'ar' ? 'الفئات الفرعية' : 'Subcategories'}
																											</div>
                                                    ...
                                                    : subcategory.name;
																													...
																													return (
																														<button
																															...
																															className={`w-full px-3 py-2 rounded-lg text-gray-600 hover:bg-teal-50 hover:text-teal-700 transition-all duration-200 text-sm ${isArabicText(subcategoryName) ? 'text-right' : 'text-left'}`}>
																															{subcategoryName}
																														</button>
																													);
																												})}
              ...
              >
                ...
                <AnimatePresence>
                    ...
                    <motion.div
                      ...
                      animate="visible"
                      exit="exit"
                      className={`language-dropdown absolute ${isRTL ? 'left-0' : 'right-0'} mt-1 w-32 rounded-xl bg-white/95 backdrop-blur-lg border border-white/30 shadow-xl p-2 z-50`}
                      onMouseEnter={() => setShowLanguageDropdown(true)}
                      onMouseLeave={() => setShowLanguageDropdown(false)}
                    >
													<button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all duration-200 ${currentLanguage === 'en' 
                      ...
                      >
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all duration-200 ${currentLanguage === 'ar' 
                            ? 'bg-teal-600 text-white' 
                            : 'text-gray-700 hover:bg-teal-50 hover:text-teal-600'
                          }`}
                      >
          ...
          <div className={`flex items-center flex-shrink-0 ${isRTL ? 'space-x-reverse space-x-2 sm:space-x-reverse sm:space-x-3' : 'space-x-2 sm:space-x-3'}`}>
            {/* Contact Icons - Only visible on XL screens and bigger */}
            <div className={`hidden xl:flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
              <motion.a
										...
										return (
											...
											>
												<Link
													...
													`}
												>
													<div className={`p-2 rounded-xl ${isActive ? 'bg-white/20' : 'bg-teal-100 group-hover:bg-teal-200'} transition-colors ${isRTL ? 'ml-3' : 'mr-3'}`}>
								...
								<div className="border-t border-gray-200/50 pt-6">
									...
									<div className="grid grid-cols-2 gap-3 mb-6">
										...
										>
											...
											<span className="text-xs font-medium">{currentLanguage === 'ar' ? 'إيميل' : 'Email'}</span>
										</motion.a>
										<motion.a
											href="tel:+971556707773"
											whileHover={{ scale: 1.05, y: -2 }}
										...
										>
											...
											<span className="text-xs font-medium">{currentLanguage === 'ar' ? 'هاتف' : 'Phone'}</span>
										</motion.a>
									</div>
