...
const ProductDetail: React.FC = () => {
    ...
    const getLocalizedCategoryName = (category: { name?: Localizable } | null | undefined): string => {
		if (!category) return "";
		return getLocalizedProductField(category.name);
	};

    // Helper: get localized text (alias for getLocalizedProductField)
    const getLocalizedText = (value: Localizable): string => {
    ...
    const resolveText = (value: Localizable, localized?: string): string => {
		...
		const loadProduct = async () => {
			...
			try {
				...
				const normalized: Product = {
                    ...
                    features: (raw.localized?.features || raw.features || []).map((f: Localizable) => resolveText(f)),
                    specifications: Object.fromEntries(
                        Object.entries(raw.specifications || {}).map(([k, v]) => [k, resolveText(v as Localizable)])
                    ),
				...
				const related = allProducts
                    .map((p: Product & { localized?: { name?: Localizable; description?: Localizable } }) => ({
						...p,
	...
	return (
		<div className='min-h-screen bg-gradient-to-br from-gray-50 to-white'>
			<div className='container mx-auto px-4  sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 pt-16 sm:pt-20 lg:pt-24'>
				...
				<div className='grid grid-cols-1 xl:grid-cols-2 mt-16 gap-4 sm:gap-6 lg:gap-8 xl:gap-12'>
					{/* Product Images */}
					<motion.div
						...
						<div className='relative overflow-hidden rounded-xl sm:rounded-2xl shadow-xl cursor-pointer'>
                            ...
                            <div className={`absolute top-2 sm:top-4 ${isRTL ? 'right-2 sm:right-4' : 'left-2 sm:left-4'}`}>
                                <span className={`inline-flex items-center space-x-1 sm:space-x-2 ${isRTL ? 'space-x-reverse' : ''} bg-white/95 backdrop-blur-sm -webkit-backdrop-blur-sm text-gray-800 px-2 sm:px-3 py-1.5 rounded-lg font-medium border border-white/20 shadow-lg text-xs sm:text-sm`}>
					...
					<motion.div
						...
						<motion.div
							...
							<div className='pt-4 border-t border-gray-200'>
                                ...
                                <button
                                    ...
                                    className={`mt-3 inline-flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'} bg-primary-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-primary-700 transition-colors text-sm holographic-card`}
						...
						<motion.div
                            ...
                            <button
                                ...
                                aria-label={product.inStock ? (isInCart(product._id) ? t('addedToCart') : t('addToCart')) : t('outOfStock')}
                                title={product.inStock ? (isInCart(product._id) ? t('addedToCart') : t('addToCart')) : t('outOfStock')}
							>
								<ShoppingCart className='w-4 mx-2 h-4 sm:w-5 sm:h-5' />
                                <span>
                                    {product.inStock 
