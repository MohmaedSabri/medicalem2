	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (isCreating) {
				...
				const original = (allSubCategories || []).find(s => s._id === editingId);
				const mergeLocalized = (value: string, originalValue: unknown) => {
					const prev = originalValue && typeof originalValue === 'object' ? originalValue as { en?: string; ar?: string } : {};
					return {
				...
				await updateSubCategory.mutateAsync({
					...
					subcategoryData: {
						...
						description: mergeLocalized(formData.description, original?.description),
						parentCategory: formData.parentCategory,
					},
				});
			...
			toast.error(t('failedToSave', { defaultValue: 'Failed to save subcategory' }));
		}
	};
		...
		<div className="space-y-6">
			...
			<div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
				<div className={`${isRTL ? 'text-right' : 'text-left'}`}>
					<h2 className={`text-2xl font-bold text-gray-900 flex items-center ${isRTL ? 'flex-row-reverse space-x-reverse space-x-2' : 'space-x-2'}`}>
			...
			<div className="bg-white rounded-xl shadow-sm border border-gray-200">
				<div className="p-6 border-b border-gray-200">
					<h3 className={`text-lg font-semibold text-gray-900 flex items-center ${isRTL ? 'flex-row-reverse space-x-reverse space-x-2' : 'space-x-2'}`}>
							...
							<div key={category._id} className="p-4">
								...
								<div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
									<div className={`flex items-center ${isRTL ? 'flex-row-reverse space-x-reverse space-x-3' : 'space-x-3'}`}>
										...
										<div className={`${isRTL ? 'text-right' : 'text-left'}`}>
											<h4 className="font-medium text-gray-900">{getLocalizedText(category.name)}</h4>
											<p className="text-sm text-gray-500">{getLocalizedText(category.description)}</p>
										</div>
									</div>
									...
									>
										{subCategories.length === 0 ? (
											...
											subCategories.map((subcategory) => (
												<div
													key={subcategory._id}
													className={`flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 ${isRTL ? 'flex-row-reverse' : ''}`}
												>
													<div className={`${isRTL ? 'text-right' : 'text-left'}`}>
														<h5 className="font-medium text-gray-900">{getLocalizedText(subcategory.name)}</h5>
														<p className="text-sm text-gray-600">{getLocalizedText(subcategory.description)}</p>
													</div>
			...
			{(isCreating || editingId) && (
				...
				>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						<div className={`flex items-center justify-end ${isRTL ? 'space-x-reverse space-x-3' : 'space-x-3'} pt-4`}>
							<button
								type="button"
								onClick={handleCancel}
