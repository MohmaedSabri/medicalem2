...
export const doctorCreateSchema = yup.object({
  ...
  image: yup.string().url("Image must be a valid URL").trim().required("Image is required"),
  ...
  specializationEn: yup.string().trim().required("Specialization (EN) is required"),
  specializationAr: yup.string().trim().required("Specialization (AR) is required"),
});
